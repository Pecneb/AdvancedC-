...
public:
	...
	Customer(int id, std::chrono::seconds taskComplexity) :
		...
		isAlreadyDone{false}
	{}
    Customer(const Customer& other) : customerId(other.customerId), 
				...
				if (cust.tryToLock()) {
                    if (cust.isWaiting() && timeAvailable > 0) {
                        ...
                        if (timeAvailable >= remainingTimeForCustomer) {
                            std::this_thread::sleep_for( std::chrono::seconds(remainingTimeForCustomer));
                            cust.setToDone();
                            timeAvailable = timeAvailable - remainingTimeForCustomer;
                        }
                        else {
                            std::this_thread::sleep_for( std::chrono::seconds(timeAvailable));
